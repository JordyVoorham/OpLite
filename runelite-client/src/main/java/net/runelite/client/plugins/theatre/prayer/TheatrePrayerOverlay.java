...
public abstract class TheatrePrayerOverlay extends Overlay
{
	...
	{
		final Widget meleePrayerWidget = client.getWidget(Varbits.PRAYER_PROTECT_FROM_MELEE);
		final Widget rangePrayerWidget = client.getWidget(Varbits.PRAYER_PROTECT_FROM_MISSILES);
		final Widget magicPrayerWidget = client.getWidget(Varbits.PRAYER_PROTECT_FROM_MAGIC);
	...
	private void renderDescendingBoxes(Graphics2D graphics)
	{
		...
		getAttackQueue().forEach(attack -> {
			...
			final Widget prayerWidget = client.getWidget(attack.getPrayer().getVarbit());
