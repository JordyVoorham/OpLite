		...
		if (sotetseg.isSotetsegActive())
		{
			...
			if (config.sotetsegOrbAttacksTicks() || config.sotetsegBigOrbTicks())
			{
				for (Projectile p : client.getProjectiles())
				{
					...
					Point point = Perspective.localToCanvas(client, new LocalPoint((int) p.getX(), (int) p.getY()), 0, Perspective.getTileHeight(client, new LocalPoint((int) p.getX(), (int) p.getY()), 0) - (int) p.getZ());
